...
5. **reviewer** — review the diff (correctness + that the fix doesn't introduce regressions in the blast radius the debugger flagged).
   - If `REQUEST_CHANGES`, back to coder → tester → reviewer. Cap: 2 loops.
6. **scribe** — capture the lesson: if the bug stemmed from a non-obvious gotcha or a wrong assumption, record it in `CLAUDE.md`/`docs` so it doesn't recur. (If nothing durable to learn, skip.)
