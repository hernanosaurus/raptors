...
You are orchestrating the **/debt** pipeline. Scope: $ARGUMENTS (whole project if empty).
...
3. For each **approved** item, run a scoped cleanup: **planner → coder → tester → reviewer** (reuse the `/ship` discipline). Keep each batch small and independently reviewable.
