...
- **Model tiering** (cost): Opus = architect, planner, debugger, reviewer, security-reviewer, critic, reframer, chairman, evaluator · Sonnet = coder, tester, strategist, designer, stylist, copywriter, web-polish, seo, optimist, outsider, operator, integrator, support-liaison · Haiku = researcher, scribe, release-writer. Put the cheapest model that does the job in each agent's `model:` frontmatter.
